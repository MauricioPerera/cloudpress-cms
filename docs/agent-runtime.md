...
4. `route_model` selecciona exclusivamente un modelo habilitado que cumpla residencia, límites de tokens y el presupuesto restante de la ejecución.
5. `invoke_model` sólo invoca Workers AI después del enrutamiento; si falta su binding en Pages, falla cerradamente sin enviar el prompt.
6. `record_model_usage` acepta únicamente un modelo habilitado del catálogo, recalcula el coste en el servidor y vuelve a validar residencia, límites de tokens y presupuesto. Los valores declarados por el runner no determinan el coste ni pueden crear telemetría de un modelo inventado.
7. `write_memory` y `recall_memories` manejan memoria episódica del mismo perfil y propietario.
8. `send_message` y `receive_messages` comunican perfiles del mismo propietario, respetando la clasificación menor común.
9. `delegate_task` crea una tarea hija para otro perfil activo del mismo propietario. La tarea padre no puede completarse mientras el hijo esté activo.
...
El catálogo de modelos conserva residencia y precios por token. El presupuesto se aplica acumulativamente por ejecución: el router suma el gasto registrado y reserva el coste estimado antes de autorizar otra inferencia. `external-webmcp` se ejecuta en el runner local; `cloudflare-workers-ai` usa `env.AI.run()` exclusivamente si Pages tiene un binding **AI** configurado en el panel de Cloudflare y el modelo fue habilitado de forma explícita. Workers AI se cataloga como `cloudflare-managed`: Cloudflare no ofrece Regional Services para Workers AI, por lo que CloudPress rechaza etiquetas geográficas como `wnam` que implicarían una garantía de residencia inexistente. Registrar uso no autoriza inferencia ni envío de datos a un proveedor. Sin ese binding, `invoke_model` falla cerradamente y no envía el prompt a ninguna red.
