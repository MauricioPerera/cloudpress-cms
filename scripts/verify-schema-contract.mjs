...
for (const table of ["content_items", "plugin_installations", "plugin_records", "plugin_webhooks", "approval_requests", "approval_events", "agent_capabilities", "totp_credentials", "totp_recovery_codes", "totp_recovery_requests"]) {
...
assert.equal(migrations.length, 17, "Actualiza esta prueba al añadir una migración nueva.");
assert.deepEqual(migrations.map((name) => Number(name.slice(0, 4))), Array.from({ length: 17 }, (_, index) => index + 2), "Las migraciones deben ser consecutivas.");
assert.equal(migrations.at(-1), "0018_agent_capabilities.sql", "Actualiza esta prueba al añadir una migración nueva.");
