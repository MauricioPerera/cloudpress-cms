...
try {
  await withMockDocument(mock, async () => {
    ...
    for (const name of ["cloudpress_record_task_step", "cloudpress_read_admin_state", "cloudpress_create_draft", "cloudpress_plugin_action", "cloudpress_upload_media", "cloudpress_sensitive_action"]) assert.equal(mock.hasTool(name), true, `${name} debe registrarse.`);
