...
import { encryptTotpSecret, recoveryCompanionProof, totpAt } from "../functions/_totp.js";
...
const env = { DB, TOTP_ENCRYPTION_KEY: encryptionKey, LSFA_RECOVERY_SIGNING_KEY: "test-lsfa-recovery-signing-key-at-least-32" };
...
assert.deepEqual({ ok: verifiedPayload.ok, state: verifiedPayload.state }, { ok: true, state: "verified" });
assert.match(verifiedPayload.requestId, /^[0-9a-f-]{36}$/i);
const proof = await recoveryCompanionProof(env, ticket.requestId, ticket.recoveryToken);
const missingProof = await complete({ request: post("/api/totp-recovery/" + ticket.requestId + "/complete", { password: "Contraseña-nueva" }, { "x-cloudpress-recovery-token": ticket.recoveryToken }), env, params: { id: ticket.requestId } });
assert.equal(missingProof.status, 403, "La recuperación requiere una prueba emitida por el companion.");
const changed = await complete({ request: post("/api/totp-recovery/" + ticket.requestId + "/complete", { password: "Contraseña-nueva" }, { "x-cloudpress-recovery-token": ticket.recoveryToken, "x-cloudpress-lsfa-recovery-proof": proof, "CF-Connecting-IP": "198.51.100.12" }), env, params: { id: ticket.requestId } });
const changedPayload = await changed.json();
assert.deepEqual({ ok: changedPayload.ok, state: changedPayload.state }, { ok: true, state: "used" });
assert.match(changedPayload.requestId, /^[0-9a-f-]{36}$/i);
...
database.close();
console.log(JSON.stringify({ ok: true, checks: ["totp-recovery-prepare", "server-otp-verification", "lsfa-proof-required", "single-use-password-reset"] }));
