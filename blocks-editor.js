(() => {
  ...
  const remember = (root, item) => { const form = root.querySelector("#content-form"); if (form) item._draft = { title: form.title.value, slug: form.slug.value, excerpt: form.excerpt.value, status: form.status.value, terms: [...root.querySelectorAll(".term:checked")].map((input) => Number(input.value)) }; };
  ...
  function bind(root, kind) {
    ...
    form.onsubmit = async (event) => { event.preventDefault(); const payload = { kind, title: form.title.value, slug: form.slug.value, excerpt: form.excerpt.value, status: form.status.value, blocks: item.blocks, termIds: [...root.querySelectorAll(".term:checked")].map((input) => Number(input.value)) }; try { if (item.id) await api(`/api/admin/entries/${item.id}`, "PATCH", payload); else await api("/api/admin/entries", "POST", payload); await loadContent(kind); state.editing = null; renderContent(kind); await loadOverview(); notify("Contenido guardado."); } catch (error) { notify(error.message, true); } };
  }
