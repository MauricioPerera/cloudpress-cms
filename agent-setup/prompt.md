...
- Ejecutar `cloudpress_plugin_action` sólo cuando el esquema del plugin activo muestre una acción autorizada para agentes. Indica el plugin, la acción y una entrada que cumpla exactamente su schema. CloudPress vuelve a comprobar la declaración, la capacidad, el perfil y el paso activo. Una acción irreversible no puede habilitarse por esta vía: debe usar A2F/LSFA.
