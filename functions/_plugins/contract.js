...
const CONTRACT_VERSION_V3 = "cloudpress-plugin/v3";
const HOOKS = new Set([
  "content.beforeCreate", "content.afterCreate",
  "content.beforeUpdate", "content.afterUpdate",
  "content.beforeTrash", "content.afterTrash",
  "content.beforeRestore", "content.afterRestore"
]);
const PERMISSIONS = new Set(["content:read", "content:transform", "content-types:define", "content-meta:define", "user-meta:define", "actions:register", "taxonomies:define", "admin-ui:register", "storage:read", "storage:write", "routes:register", "jobs:enqueue", "privacy:manage", "diagnostics:read", "capabilities:define", "webhooks:register", "blocks:define"]);
...
const validRole = (value) => idPattern.test(String(value || ""));
...
const validScopedTypeId = (pluginId, value) => /^[a-z0-9][a-z0-9-]{2,47}--[a-z0-9][a-z0-9-]{1,47}$/.test(String(value || "")) && String(value).startsWith(`${pluginId}--`);
const validActionSchema = (schema) => {
  if (!isObject(schema) || schema.type !== "object" || !isObject(schema.properties) || !uniqueStrings(schema.required || [])) return false;
  if (schema.additionalProperties !== undefined && typeof schema.additionalProperties !== "boolean") return false;
  if (JSON.stringify(schema).length > 8000) return false;
  return Object.entries(schema.properties).every(([key, rule]) => /^[a-zA-Z][a-zA-Z0-9_-]{0,63}$/.test(key) && isObject(rule) && ["string", "number", "boolean"].includes(rule.type) && (rule.minLength === undefined || Number.isInteger(rule.minLength) && rule.minLength >= 0 && rule.minLength <= 4000) && (rule.maxLength === undefined || Number.isInteger(rule.maxLength) && rule.maxLength >= 0 && rule.maxLength <= 4000 && (rule.minLength === undefined || rule.minLength <= rule.maxLength)) && (rule.minimum === undefined || Number.isFinite(rule.minimum)) && (rule.maximum === undefined || Number.isFinite(rule.maximum) && (rule.minimum === undefined || rule.minimum <= rule.maximum)) && (rule.pattern === undefined || typeof rule.pattern === "string" && rule.pattern.length <= 200));
};
...
export function validatePluginManifest(input) {
  ...
  if (![CONTRACT_VERSION, CONTRACT_VERSION_V2, CONTRACT_VERSION_V3].includes(input.contractVersion)) errors.push(`contractVersion debe ser ${CONTRACT_VERSION}, ${CONTRACT_VERSION_V2} o ${CONTRACT_VERSION_V3}.`);
  ...
  if (input.priority !== undefined && (!Number.isInteger(input.priority) || input.priority < -100 || input.priority > 100)) errors.push("priority debe ser un entero entre -100 y 100.");
  ...
  if (Array.isArray(input.hooks) && ["content.beforeCreate", "content.beforeUpdate"].some((hook) => input.hooks.includes(hook)) && !input.permissions?.includes("content:transform")) errors.push("Los hooks de transformación de contenido exigen el permiso content:transform.");
  ...
  const validTypeId = (value) => input.contractVersion === CONTRACT_VERSION_V3 ? validScopedTypeId(input.id, value) : idPattern.test(value);
  const validType = (x) => isObject(x) && validTypeId(x.id) && typeof x.label === "string" && x.label.length <= 80 && uniqueStrings(x.supports) && x.supports.every((v) => ["title","body","excerpt"].includes(v)) && (x.publicApi === undefined || typeof x.publicApi === "boolean");
  ...
  const validMeta = (x) => isObject(x) && typeof x.key === "string" && /^[a-z0-9-]+\.[a-z0-9_.-]{1,63}$/.test(x.key) && (input.contractVersion !== CONTRACT_VERSION_V3 || x.key.startsWith(`${input.id}.`)) && ["string","number","boolean","json"].includes(x.type) && typeof x.required === "boolean" && validMetaSchema(x.schema);
  const validAgentAction = (value) => value === undefined || (isObject(value) && ["read", "reversible"].includes(value.risk) && JSON.stringify(value).length <= 200);
  const validAction = (x) => isObject(x) && idPattern.test(x.id) && typeof x.label === "string" && ["content","user","site"].includes(x.scope) && validAgentAction(x.agent) && (input.contractVersion !== CONTRACT_VERSION_V3 || validActionSchema(x.inputSchema));
  const validBlock = (x) => isObject(x) && validScopedTypeId(input.id, x.id) && typeof x.label === "string" && x.label.length <= 80 && (x.icon === undefined || typeof x.icon === "string" && x.icon.length <= 8) && typeof x.handler === "string" && idPattern.test(x.handler) && validActionSchema(x.attributes);
  ...
  declarations(input.blocks, "blocks:define", validBlock, "blocks", errors);
  ...
  if (input.blocks?.length && !input.permissions?.includes("blocks:define")) errors.push("blocks exige blocks:define.");
  if ([CONTRACT_VERSION_V2, CONTRACT_VERSION_V3].includes(input.contractVersion) && input.actions?.some((action) => typeof action?.handler !== "string" || !idPattern.test(action.handler) || !validCapability(action.capability))) errors.push("Cada acción v2/v3 exige handler y capability válidos.");
  const validRoute = (x) => isObject(x) && typeof x.path === "string" && /^\/[a-z0-9/_-]{0,120}$/.test(x.path) && uniqueStrings(x.methods) && x.methods.length && x.methods.every((method) => ["GET", "POST", "PATCH", "PUT", "DELETE"].includes(method)) && typeof x.handler === "string" && idPattern.test(x.handler) && validCapability(x.capability) && (x.public === undefined || x.public === true && x.methods.every((method) => method === "GET"));
  ...
  const validTaxonomy = (x) => isObject(x) && idPattern.test(x.id) && typeof x.label === "string" && typeof x.hierarchical === "boolean" && uniqueStrings(x.objectTypes) && x.objectTypes.every((item) => ["post", "page"].includes(item) || validTypeId(item));
  ...
  requires(input, "routes", "routes:register", errors); requires(input, "tasks", "jobs:enqueue", errors); requires(input, "menus", "admin-ui:register", errors); requires(input, "taxonomies", "taxonomies:define", errors); requires(input, "migrations", "storage:write", errors); requires(input, "capabilities", "capabilities:define", errors); requires(input, "webhooks", "webhooks:register", errors); requires(input, "blocks", "blocks:define", errors);
  if (input.contractVersion === CONTRACT_VERSION && ["routes", "tasks", "menus", "taxonomies", "migrations", "capabilities", "webhooks", "blocks"].some((key) => input[key]?.length)) errors.push("Estas declaraciones requieren cloudpress-plugin/v2.");
  return { valid: errors.length === 0, errors };
}

export { CONTRACT_VERSION, CONTRACT_VERSION_V2, CONTRACT_VERSION_V3, HOOKS, PERMISSIONS };
