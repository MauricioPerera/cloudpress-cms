...
export function validatePluginManifest(input) {
  ...
  const validType = (x) => isObject(x) && validTypeId(x.id) && typeof x.label === "string" && x.label.length <= 80 && uniqueStrings(x.supports) && x.supports.every((v) => ["title","body","excerpt"].includes(v));
