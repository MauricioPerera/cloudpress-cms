...
export async function onRequestGet({ request, env }) {
  ...
  const [types, meta, actions] = await env.DB.batch([
  ...
  return json({ contentTypes: types.results, meta: meta.results, actions: actions.results }, 200, { "Cache-Control": "no-store" });
}
