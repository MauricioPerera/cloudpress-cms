...
export async function onRequestPost({ request, env, params }) {
  ...
  try { const result = await handler(createPluginContext(env, pluginId, admin), JSON.parse(job.payload_json)); await env.DB.prepare("UPDATE plugin_jobs SET status='completed',result_json=?,updated_at=? WHERE id=?").bind(JSON.stringify(result ?? null), new Date().toISOString(), job.id).run(); await pluginAudit(env, pluginId, "job_completed", admin.id, { jobId: job.id, taskId: job.task_id }); return json({ ok: true, job: { id: job.id, taskId: job.task_id, result } }); }
  catch (error) { await env.DB.prepare("UPDATE plugin_jobs SET status='failed',error_text=?,updated_at=? WHERE id=?").bind(String(error?.message || "Error"), new Date().toISOString(), job.id).run(); await pluginAudit(env, pluginId, "job_failed", admin.id, { jobId: job.id, taskId: job.task_id }); return json({ error: String(error?.message || "La tarea falló.") }, 422); }
}
