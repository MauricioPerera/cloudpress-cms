...
export async function onRequestPost({ request, env }) {
  ...
  await env.DB.batch([
    env.DB.prepare("DELETE FROM plugin_content_types WHERE plugin_id=?").bind(id), env.DB.prepare("DELETE FROM plugin_meta_definitions WHERE plugin_id=?").bind(id), env.DB.prepare("DELETE FROM plugin_actions WHERE plugin_id=?").bind(id),
  ...
  return json({ ok: true, id, status: "enabled", verdict }, 201);
}
