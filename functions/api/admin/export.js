...
export async function onRequestGet({ request, env }) {
  ...
  const [content, terms, relations, menu, comments, revisions, settings] = await env.DB.batch([
    ...
    env.DB.prepare("SELECT content_items.slug AS content_slug,comments.author_name,comments.author_email,comments.body,comments.status,comments.created_at,comments.updated_at FROM comments JOIN content_items ON content_items.id=comments.content_id ORDER BY comments.id"),
  ...
  const payload = { format: "cloudpress-export", version: 2, exportedAt: new Date().toISOString(), content: content.results, terms: terms.results, contentTerms: relations.results, menu: menu.results, comments: comments.results, revisions: revisions.results, settings: settings.results };
