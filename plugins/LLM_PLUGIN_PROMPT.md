...
- `actions` requiere `actions:register`. Cada acción tiene `id`, `label`, `scope`, `handler`, `capability` e `inputSchema`; en v3 el schema es un objeto estricto, con propiedades primitivas declaradas, `required` y `additionalProperties: false` cuando no se admitan campos extra. Para permitirla a un agente, declara explícitamente `agent: { "risk": "read" }` o `agent: { "risk": "reversible" }`; el core exige además el perfil, la capacidad del plugin y un paso activo de tarea. Nunca declares una acción irreversible para agente: no es válida y debe modelarse mediante A2F/LSFA. El handler vive en `export default { actions: { ... } }`.
